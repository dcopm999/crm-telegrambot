...
def coverage(c):
    ...
    """
    c.run("coverage run --source vacancy-telegrambot runtests.py tests")
...
def docs(c):
    ...
    """
    c.run("rm -f docs/vacancy-telegrambot.rst")
    c.run("rm -f docs/modules.rst")
    c.run("sphinx-apidoc -o docs/ telegrambot")
...
def lint(c):
    ...
    """
    c.run("flake8 vacancy-telegrambot tests")
...
def release(c, bumpsize=''):
    ...
    import vacancy_telegrambot
    c.run("python setup.py sdist bdist_wheel")
    c.run("twine upload dist/*")

    c.run('git tag -a {version} -m "New version: {version}"'.format(version=vacancy_telegrambot.__version__))
    c.run("git push --tags")
