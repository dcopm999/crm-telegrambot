...
from telegrambot.mixins import CatalogMixin, HistoryMixin
...
class HandlerCommandBase(AbstractHandler):
    ...
    def handle(self, request: Any) -> str:
        ...
        if (
            request.get("callback_query", False)
            or request.get("message").get("text") == self.command  # noqa: W503
        ):
            self.run(request)
        ...
        if request.get("message"):
            ...
            if products is None:
                ...
                for product in products:
                    ...
                    self.bot.send_photo(
                        chat_id, product.image.path, product.caption, buy_btn
                    )
